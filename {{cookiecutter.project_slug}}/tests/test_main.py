...
def test_save_config_file_as() -> None:
    ...
    with test_save_config_filepath.open() as fp:
        ...
        assert "loglevel" in data["{{cookiecutter.project_slug}}"]
        assert data["{{cookiecutter.project_slug}}"]["loglevel"] == "DEBUG"
...
def test_save_config_file() -> None:
    ...
    with test_save_config_filepath.open() as fp:
        data = tomlkit.load(fp).value
        assert "loglevel" in data["{{cookiecutter.project_slug}}"]
        assert data["{{cookiecutter.project_slug}}"]["loglevel"] == "ERROR"
